///////////////////////////////////////////////////////////////////////////////////////////////////
//// root -l ./tmvaClass.C\(\"750.\",\"1450.\",\"2110.\",\"2500.\",\"reffinal OR muons\",\"BDTG\"\)
///////////////////////////////////////////////////////////////////////////////////////////////////


// @(#)root/tmva $Id: tmvaClass.C 44112 2012-05-04 10:00:41Z evt $
/**********************************************************************************
* Project   : TMVA - a ROOT-integrated toolkit for multivariate data analysis    *
* Package   : TMVA                                                               *
* Root Macro: tmvaClass                                                          *
*                                                                                *
...
*                                                                                *
*    root -l ./tmvaClass.C\(\"Fisher,Likelihood\"\)                              *
*                                                                                *
...
void tmvaClass( TString mMinSBleft, TString mMaxSBleft, TString mMinSBright, TString mMaxSBright, TString metType, TString myMethodList = "" )
{
	...
	//
	// mylinux~> root -l tmvaClass.C.C\(\"myMethod1,myMethod2,myMethod3\"\)
	//
	// if you like to use a method via the plugin mechanism, we recommend using
	//
	// mylinux~> root -l tmvaClass.C.C\(\"P_myMethod\"\)
	...
	TMVA::Factory *factory = new TMVA::Factory( "tmvaClass", outputFile,"!V:!Silent:Color:DrawProgressBar:Transformations=I;D;P;G,D:AnalysisType=Classification" );
	...
	*/
	...
	// factory->AddVariable( "vtx_isolation003", "Isolation_{03}", "",     'F' );
	// factory->AddVariable( "vtx_isolation010", "Isolation_{10}", "",     'F' );
	// factory->AddVariable( "vtx_isolation014", "Isolation_{14}", "",     'F' );
	factory->AddVariable( "vtx_isolation020", "Isolation_{20}", "",     'F' );
	factory->AddVariable( "vtx_pval",         "#it{p}-value",   "",     'F' );
	factory->AddVariable( "trks_fitprob",     "Trkfit prob.",   "",     'F' );
	factory->AddVariable( "vtx_pt",           "p_{T}^{3body}",  "MeV",  'F' );
	factory->AddVariable( "ht_pt",            "#it{H}_{T}",     "MeV",  'F' );
	factory->AddVariable( "mets_dphi",        "#Delta#phi(#it{E}_{T,trk}^{miss},#it{E}_{T,cal}^{miss})", "", 'F' );
	
	
	factory->AddVariable( "met_"+metType+"_et",      "#it{E}_{T,cal}^{miss}",                 "MeV", 'F' );
	factory->AddVariable( "met_"+metType+"_mT",      "#it{m}_{T}^{cal}",                      "MeV", 'F' );
	factory->AddVariable( "mets_dptrelcal",       "p_{T}^{3body}/#it{E}_{T,cal}^{miss}-1", "",    'F' );
	// factory->AddVariable( "mets_dhtrelcal",          "#it{H}_{T}/#it{E}_{T,cal}^{miss}-1", "",    'F' );
	factory->AddVariable( "met_track_et",            "#it{E}_{T,trk}^{miss}",                 "MeV", 'F' );
	factory->AddVariable( "met_track_mT",            "#it{m}_{T}^{trk}",                      "MeV", 'F' );
	factory->AddVariable( "mets_dptreltrk",       "p_{T}^{3body}/#it{E}_{T,trk}^{miss}-1", "",    'F' );
	// factory->AddVariable( "mets_dhtreltrk",          "#it{H}_{T}/#it{E}_{T,trk}^{miss}-1", "",    'F' );

	factory->AddVariable( "met_"+metType+"_dPhi3mu", "#Delta#phi(3body,#it{E}_{T,cal}^{miss})", "",'F' );
	// factory->AddVariable( "met_track_dPhi3mu",       "#Delta#phi(3body,#it{E}_{T,trk}^{miss})", "",'F' );
	// factory->AddVariable( "ht_dr3body",    "#DeltaR(#it{H}_{T},3body)", "",    'F' );
	factory->AddVariable( "geo_lxySig",       "#it{S}(#it{L}_{xy})",       "", 'F' );
	factory->AddVariable( "geo_a0xySig",      "#it{S}(#it{a}^{0}_{xy})",   "", 'F' );
	factory->AddVariable( "vtx_pvNtrk",       "N_{trk}^{PV}",   "",     'I' );

	// factory->AddVariable( "ht_mT",               "#it{m}_{T}(#it{H}_{T},#it{E}_{T,cal}^{miss})",  "MeV", 'F' );
	// factory->AddVariable( "ht_dphimet_"+metType, "#Delta#phi(#it{H}_{T},#it{E}_{T,cal}^{miss})",  "",    'F' );
	// factory->AddVariable( "ht_mT_mettrk",        "#it{m}_{T}(#it{H}_{T},#it{E}_{T,trk}^{miss})", "MeV", 'F' );
	// factory->AddVariable( "ht_dphimet_track",    "#Delta#phi(#it{H}_{T},#it{E}_{T,trk}^{miss})", "",    'F' );
	
	
	// factory->AddVariable( "muons_maxpbalsig", "#it{#sigma}_{p-bal}^{max}", "", 'F' );
	// factory->AddVariable( "vtx_code",         "Code",                      "", 'I' );
	//factory->AddVariable( "vtx_lxy",          "#it{L}_{xy}",             "",     'F' );
	//factory->AddVariable( "vtx_lxyErr",       "#Delta#it{L}_{xy}",       "",     'F' );
	//factory->AddVariable( "vtx_a0xy",         "#it{a}^{0}_{xy}",         "#mum", 'F' );
	//factory->AddVariable( "vtx_a0xyErr",      "#Delta#it{a}^{0}_{xy}",   "#mum", 'F' );


	// factory->AddVariable( "vtx_mOS1",      "#it{m}_{OS1}",   "MeV", 'F' );
	// factory->AddVariable( "vtx_mOS2",      "#it{m}_{OS2}",   "MeV", 'F' );
	// factory->AddVariable( "vtx_mSS",      "#it{m}_{SS}",   "MeV", 'F' );


	
	// factory->AddSpectator( "vtx_mOS1",      "#it{m}^{OS1}",   "MeV", 'F' );
	// factory->AddSpectator( "vtx_mOS2",      "#it{m}^{OS2}",   "MeV", 'F' );
	// factory->AddSpectator( "vtx_mSS",       "#it{m}^{SS}",    "MeV", 'F' );
	...
	*/
	...
	if(gSystem->AccessPathName( fname )) gSystem->Exec("wget http://root.cern.ch/files/tmva_class_example.root"); 
	TFile *input = TFile::Open( fname );
	std::cout << "--- tmvaClass       : Using input file: " << input->GetName() << std::endl;
	// --- Register the training and test trees
	TTree *signal     = (TTree*)input->Get("TreeS");
	TTree *background = (TTree*)input->Get("TreeB");
	*/
	TString fname = "flatout.periodall+MC.muons.cuts.analysis.n0.j0.loose.root";
	TFile* input = TFile::Open( fname );
	std::cout << "--- tmvaClass : Using input file: " << input->GetName() << std::endl;
	// --- Register the training and test trees
	TTree* signal     = (TTree*)input->Get("flatout_Wtaunu_3mu");
	TTree* background = (TTree*)input->Get("flatout_Data");
	...
	*/
	factory->SetSignalWeightExpression("wgt_pileup");
	

	// Apply additional cuts on the signal and background samples (can be different)
	// for example: TCut mycuts = "abs(var1)<0.5 && abs(var2-0.5)<1";
	// for example: TCut mycuts = "vtx_mass>1300. && vtx_mass<2300.";
	...
	TCut    kinematics = ("vtx_pt>10000 && (met_"+metType+"_et>10000 && met_"+metType+"_et<250000) && (met_track_et>10000 && met_track_et<250000)  &&  met_"+metType+"_mT>20000 && met_track_mT>20000").Data();
	TCut    iso        = "vtx_isolation020<0.3 && vtx_isolation030<1";

	TCut mycuts = isLoose+oneCand+masses2+iso+distances+pvalues+kinematics; // distances+pvalues+kinematics+iso
	TCut mycutb = isLoose+oneCand+masses2+sidebandsOnly+iso+distances+pvalues+kinematics; // distances+pvalues+kinematics+iso+oneCand
	...
	factory->PrepareTrainingAndTestTree( mycuts, mycutb, "nTrain_Signal=6000:nTrain_Background=4000:SplitMode=Random:NormMode=NumEvents:!V" );
                                                                      //6649//               //4672//
